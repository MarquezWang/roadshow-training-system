...
export async function POST(
  ...
  try {
    ...
    const [aiContext, template, transcript, pitchAnalysis] = await Promise.all([
    ...
    return NextResponse.json({
      questions: await getExistingQuestions(session.id),
    });
  } catch (error) {
    ...
    console.error("答辩问题生成失败。", { error: message });
